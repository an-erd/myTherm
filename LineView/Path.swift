...
extension Path {
    ...
    static func lineChart(points: [Double], step: CGPoint, offset: CGFloat) -> Path {
        var path = Path()
        if (points.count < 2){
            return path
        }
        let p1 = CGPoint(x: 0, y: CGFloat( points[0] - Double(offset) ) * step.y)
        path.move(to: p1)
//        print("linechart * \(p1) y \(CGFloat(points[0])) offset \(offset) step.y \(step.y)")
        for pointIndex in 1..<points.count {
            let p2 = CGPoint(x: step.x * CGFloat(pointIndex),
                             y: step.y * CGFloat(points[pointIndex] - Double(offset)))
            path.addLine(to: p2)
